...
public class LoginCheck extends HttpServlet {
	...
	protected void process(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 생성은 코드로 개발자가, 데이터 저장도 개발자가 단 생성된 객체관리는 컨테이너 내부 로직
		 * 
		 * request.getSession() :없을 경우 생성, 이미 존재할경우 제공
		 */
		...
		session.setAttribute("key2", 10);// new Integer(10)으로 내부적으로 변환
		
// *****************************************************************************
		//step01 쿠키 생성
		// Cookie 기술로 데이터 두가지 저장
		// 실행 후 브라우저에 쿠키 메모리 확인 
		Cookie msg1 = new Cookie("msg1", "hi");
		Cookie msg2 = new Cookie("msg2", "fisa");
		
		msg1.setMaxAge(60*60*24); //초단위 설정 ->24시간
